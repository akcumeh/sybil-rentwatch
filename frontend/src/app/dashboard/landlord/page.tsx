...
export default async function LandlordDashboardPage() {
    ...
    const scoreBreakdown = [
        { title: 'Maintenance Responsiveness', score: profile?.section_1_score ?? 0, maxScore: 250, statusLabel: '' },
        { title: 'Deposit Integrity',           score: profile?.section_2_score ?? 0, maxScore: 250, statusLabel: '' },
        { title: 'Property Accuracy',           score: profile?.section_3_score ?? 0, maxScore: 250, statusLabel: '' },
        { title: 'Behavioral Rating',           score: profile?.section_4_score ?? 0, maxScore: 250, statusLabel: '' },
    ];
    ...
    const { data: properties } = await supabase
        .from('properties')
        .select('id, address, lga, annual_rent_naira, status, bedrooms, type')
    ...
    const leaseIds = (
        await supabase.from('leases').select('id').eq('landlord_id', profile?.id)
    ).data?.map(l => l.id) ?? [];

    const { data: transactions } = leaseIds.length > 0
        ? await supabase
              .from('payments')
              .select('id, tenant_id, amount_naira, paid_date, status, payment_type')
                                ...
                                {safeProperties.slice(0, 6).map((prop, i) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-8 py-4 font-mono text-text-secondary">
                                            {formatCurrency(prop.annual_rent_naira)}
                                        </td>
                            ...
                            <tbody>
                                {transactions.map((tx, i) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-8 py-4 font-mono text-text-primary">{formatCurrency(tx.amount_naira)}</td>
                                        <td className="px-8 py-4 text-text-secondary font-mono text-xs">{formatDate(tx.paid_date)}</td>
            ...
            <div className="lg:col-span-4 flex flex-col gap-8">
                ...
                <div className="bg-surface-0/60 backdrop-blur-md border border-border-subtle p-8 rounded-sm shadow-2xl flex flex-col gap-6">
                    ...
                    <p className="font-body text-xs text-text-secondary leading-relaxed">
                        Improve your landlord score and maintain Clear Hue standing.
                    </p>
