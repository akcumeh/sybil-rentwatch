 ...
 *
 * Creates auth accounts for Landlord 1 (Adeyemi) and Tenant 3 (Kingsley),
 * then seeds the full demo dataset including properties, leases, payments,
 * disputes, ratings, and hue_score_history.
...
async function upsertAuthUser(email: string, password: string, appUserId: string) {
  // Check if auth user already exists by listing (admin API)
...
async function main() {
  ...
  // ── Step 1: Auth users for demo logins ──────────────────────────────────────
  ...
  // ── Step 6: Dispute (for T3 property care score = 210) ───────────────────────
  ...
  // ── Step 7: Rating (L1 rated T3 after first year) ────────────────────────────
  console.log('\nStep 7: Seeding ratings...')
  // T3 has no post-lease ratings yet (active lease), so behavioural = default 200
  // Nothing to insert for T3's rating on active lease
  // For T1 completion of LS2, L1 rated T1 5 stars (part of T1's score)
    {
